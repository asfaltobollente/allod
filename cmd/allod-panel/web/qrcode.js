// Pure JavaScript QR Code Generator (Zero Dependencies, Offline)
// Based on Kazuhiko Arase's QR Code Library (MIT License)
...
var QRCode = (function() {
  ...
  // Helper API to generate SVG directly
  return {
      ...
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            var x = c * cellSize;
            var y = r * cellSize;
            svg += '<rect x="' + x.toFixed(2) + '" y="' + y.toFixed(2) + '" width="' + (cellSize + 0.1).toFixed(2) + '" height="' + (cellSize + 0.1).toFixed(2) + '" fill="#000000"/>';
          }
        }
      }
      svg += '</svg>';
