...
Media files copied over the Samba network share (`\\allod\shares`) are accessible inside Jellyfin in two locations:

* **`/shares`**: Mounts the entire network share root (movies, music, TV shows, or custom directories).
* **`/media`**: Dedicated media directory (`/shares/media`).
